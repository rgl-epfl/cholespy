...
def test_cube_float():
    ...
    data = np.array([7, -1, -1, -1, -1, -1, -1, 5, -1, -1, -1, 5, -1, -1, -1, 5, -1, 5, -1, -1, -1, 5, -1, 5, -1, 7])
    assert(np.all(L.data() == data))

    rows = np.array([0, 1, 2, 3, 4, 5, 6, 1, 3, 5, 7, 2, 3, 6, 7, 3, 7, 4, 5, 6, 7, 5, 7, 6, 7, 7])
    assert(np.all(L.rows() == rows))

    col_ptr = np.array([0, 7, 11, 15, 17, 21, 23, 25, 26])
